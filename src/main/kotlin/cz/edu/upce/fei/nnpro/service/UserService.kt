...
class UserService {
    ...
    fun save(userDto: UserDto){
        if(userDto.password.isNotBlank()){
            ...
            userDto.password =  userRepository.getById(userDto.id).password
        }
        userRepository.save(userDto.toModel())
    }

    fun getAllUsers(): List<UserDto> {
        return userRepository.findAll().map { user -> user.toDto() }.filter { userDto -> userDto.username != getLoggedUser()?.username }
    }
